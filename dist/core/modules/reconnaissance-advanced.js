...
exports.runAdvancedReconnaissance = runAdvancedReconnaissance;
const promises_1 = __importDefault(require("dns/promises"));
const fingerprinting_1 = require("./recon/fingerprinting");
const cdn_waf_detection_1 = require("./recon/cdn-waf-detection");
const subdomain_enum_1 = require("./recon/subdomain-enum");
const info_disclosure_1 = require("./recon/info-disclosure");
/**
 ...
 */
async function runAdvancedReconnaissance(hostname, config) {
    const findings = [];
    ...
    try {
        const [mx, txt, ns, a, aaaa] = await Promise.all([
            promises_1.default.resolveMx(hostname).catch(() => []),
            promises_1.default.resolveTxt(hostname).catch(() => []),
            promises_1.default.resolveNs(hostname).catch(() => []),
            promises_1.default.resolve4(hostname).catch(() => []),
            promises_1.default.resolve6(hostname).catch(() => []),
        ]);
        ...
        for (const selector of dkimSelectors) {
            try {
                const dkim = await promises_1.default.resolveTxt(`${selector}._domainkey.${hostname}`);
                if (dkim.length > 0) {
                    dkimFound = true;
                    break;
                }
    ...
    if (config.recon?.technologyFingerprinting) {
        try {
            ...
            const fpResult = await (0, fingerprinting_1.performFingerprinting)(url, config);
    ...
    if (config.recon?.cdnWafDetection) {
        try {
            const url = `https://${hostname}`;
            const cdnWafResult = await (0, cdn_waf_detection_1.detectCDNAndWAF)(url, config);
    ...
    if (config.recon?.subdomainEnumeration) {
        try {
            const subdomainResult = await (0, subdomain_enum_1.enumerateSubdomains)(hostname, config);
    ...
    if (config.recon?.informationDisclosure) {
        try {
            const url = `https://${hostname}`;
            const infoResult = await (0, info_disclosure_1.checkInfoDisclosure)(url, config);
