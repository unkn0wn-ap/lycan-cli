...
exports.runAdvancedCookies = runAdvancedCookies;
...
async function runAdvancedCookies(hostname, config) {
...
function checkSecurityFlags(cookie) {
    const findings = [];
    // Check Secure flag
    if (!cookie.secure) {
        const isSensitive = SENSITIVE_COOKIE_NAMES.some(name => cookie.name.toLowerCase().includes(name));
...
function checkSensitiveData(cookie) {
    const findings = [];
...
function checkCookiePrefixes(cookie) {
    const findings = [];
...
function checkCookieScope(cookie, hostname) {
    const findings = [];
...
function checkSessionCookie(cookie) {
    const findings = [];
    const isSensitive = SENSITIVE_COOKIE_NAMES.some(name => cookie.name.toLowerCase().includes(name));
    if (isSensitive) {
