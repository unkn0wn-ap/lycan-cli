...
exports.runApiSecurity = runApiSecurity;
...
async function runApiSecurity(hostname, config) {
...
async function testCORS(baseUrl, endpoint) {
    const findings = [];
    try {
        const response = await axios_1.default.options(`${baseUrl}${endpoint}`, {
            headers: {
...
async function testRateLimiting(baseUrl, endpoint, config) {
    const findings = [];
    ...
    for (const path of graphqlPaths) {
        try {
          ...
          __schema {
            ...
            const response = await axios_1.default.post(`${baseUrl}${path}`, introspectionQuery, {
                headers: {
...
async function testApiDocumentation(baseUrl) {
    const findings = [];
    const docPaths = [
        '/api-docs',
        '/swagger',
