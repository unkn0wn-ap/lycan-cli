...
exports.runAdvancedSsrf = runAdvancedSsrf;
...
async function runAdvancedSsrf(hostname, config) {
...
async function testUrlParameters(baseUrl, config) {
    const findings = [];
    ...
    for (const param of paramsToTest) {
        ...
        try {
            const response = await axios_1.default.get(`${baseUrl}${testEndpoint}`, {
                params: {
...
async function testSsrfEndpoints(baseUrl, config) {
    const findings = [];
    ...
    for (const endpoint of endpointsToTest) {
        ...
        try {
            const checkResponse = await axios_1.default.post(`${baseUrl}${endpoint}`, {}, {
                validateStatus: () => true,
                timeout: 5000,
            });
        ...
        for (const param of ['url', 'uri', 'callback']) {
            try {
                ...
                const hasStrongSSRFEvidence = (/SSH-\d\.\d|OpenSSH_\d|ssh-rsa|SSH Protocol/i.test(responseData) ||
                ...
                try {
                    const baselineResponse = await axios_1.default.post(`${baseUrl}${endpoint}`, {
                        [param]: '',
                    }, {
                        validateStatus: () => true,
                        timeout: 5000,
...
async function testOpenRedirects(baseUrl, config) {
    const findings = [];
