...
exports.runAdvancedXxe = runAdvancedXxe;
...
async function runAdvancedXxe(hostname, config) {
...
async function testXxeInjection(baseUrl, config) {
    const findings = [];
    ...
    for (const endpoint of endpointsToTest) {
        ...
        try {
            const checkResponse = await axios_1.default.post(`${baseUrl}${endpoint}`, '<test/>', {
        ...
        try {
            const response = await axios_1.default.post(`${baseUrl}${endpoint}`, XXE_BASIC_PAYLOAD, {
                headers: {
        ...
        // Test SSRF via XXE
        try {
            const response = await axios_1.default.post(`${baseUrl}${endpoint}`, XXE_SSRF_PAYLOAD, {
                headers: {
                    'Content-Type': 'application/xml',
                },
                validateStatus: () => true,
        ...
        try {
            const response = await axios_1.default.post(`${baseUrl}${endpoint}`, XXE_PARAMETER_ENTITY, {
                headers: {
                    'Content-Type': 'application/xml',
                },
                validateStatus: () => true,
...
async function testSoapXxe(baseUrl, config) {
    const findings = [];
    const soapEndpoints = ['/soap', '/api/soap', '/ws', '/webservice'];
    for (const endpoint of soapEndpoints) {
        try {
            const response = await axios_1.default.post(`${baseUrl}${endpoint}`, SOAP_XXE_PAYLOAD, {
                headers: {
...
async function testXmlBomb(baseUrl, config) {
    const findings = [];
    const testEndpoint = '/api/xml';
    try {
        const startTime = Date.now();
        const response = await axios_1.default.post(`${baseUrl}${testEndpoint}`, XXE_BILLION_LAUGHS, {
            headers: {
                'Content-Type': 'application/xml',
            },
            validateStatus: () => true,
