...
export async function runAdvancedCsrf(
  hostname: string,
...
async function checkCookieSameSite(baseUrl: string, hostname: string): Promise<Finding[]> {
  const findings: Finding[] = [];
...
async function testStateChangingEndpoints(
  baseUrl: string,
  config: ScanConfiguration
): Promise<Finding[]> {
  const findings: Finding[] = [];
...
async function checkFormProtection(
  baseUrl: string,
  config: ScanConfiguration
): Promise<Finding[]> {
  const findings: Finding[] = [];
  ...
  for (const formPath of formsToCheck) {
    try {
      const response = await axios.get(`${baseUrl}${formPath}`, {
        validateStatus: () => true,
        timeout: 8000,
      });
...
async function testRefererValidation(
  baseUrl: string,
  config: ScanConfiguration
): Promise<Finding[]> {
  const findings: Finding[] = [];
