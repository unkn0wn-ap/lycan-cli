...
const SECRET_PATTERNS = [
  ...
  { name: 'AWS Secret Key', regex: /aws_secret_access_key\s*=\s*[\w\/\+]{40}/, severity: 'critical' as const },
  { name: 'Google API Key', regex: /AIza[0-9A-Za-z\-_]{35}/, severity: 'critical' as const },
  { name: 'GitHub Token', regex: /gh[pousr]_[A-Za-z0-9_]{36}/, severity: 'critical' as const },
  { name: 'Private Key', regex: /-----BEGIN (RSA|DSA|EC|OPENSSH) PRIVATE KEY-----/, severity: 'critical' as const },
  { name: 'Stripe API Key', regex: /sk_live_[0-9a-zA-Z]{24}/, severity: 'critical' as const },
  { name: 'Generic API Key', regex: /api[_-]?key\s*[:=]\s*['"][a-zA-Z0-9]{20,}['"]/, severity: 'high' as const },
  { name: 'Password in Code', regex: /password\s*[:=]\s*['"][^'"]{3,}['"]/, severity: 'high' as const },
  { name: 'JWT Token', regex: /eyJ[A-Za-z0-9_-]+\.eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+/, severity: 'medium' as const },
];

export async function runInfoDisclosure(
  hostname: string,
...
async function testSensitiveFile(baseUrl: string, file: string): Promise<Finding[]> {
  const findings: Finding[] = [];
...
async function testDirectoryListings(baseUrl: string): Promise<Finding[]> {
  const findings: Finding[] = [];
...
async function scanForSecrets(baseUrl: string): Promise<Finding[]> {
  const findings: Finding[] = [];
...
async function testBackupFiles(baseUrl: string): Promise<Finding[]> {
  const findings: Finding[] = [];
  ...
  for (const file of testFiles) {
    for (const ext of backupExtensions) {
      try {
        ...
        const response = await axios.get(`${baseUrl}/${backupFile}`, {
          validateStatus: () => true,
          timeout: 5000,
          maxRedirects: 0,
        });
